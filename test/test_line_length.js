...
describe('LineLength', function () {
  it('flags a line wider than the limit', function () {
    const doc = new Markdown('x.md', `# H\n${'z'.repeat(90)}`).document();
    assert.strictEqual(
      new LineLength(80).violations(doc).length, 1,
      'a 90-symbol line must raise exactly one length violation'
    );
  });
  it('accepts lines within the limit', function () {
    const doc = new Markdown('x.md', '# H\nShut gate quietly').document();
    assert.strictEqual(
      new LineLength(80).violations(doc).length, 0,
      'lines within the limit must raise no violation'
    );
  });
  it('points the violation one column past the limit', function () {
    const doc = new Markdown('x.md', 'y'.repeat(85)).document();
    assert.strictEqual(
      new LineLength(80).violations(doc)[0]
        .sarif().locations[0].physicalLocation.region.startColumn, 81,
